...
def create_passes(testbed: falcor.Testbed, max_bounces: int, use_war: bool):
    ...
    render_graph = testbed.create_render_graph("TestGauss")
    ...
    gauss_diff_pt_pass = render_graph.create_pass(
        "TestGauss",
        "TestGauss",
        {
            "samplesPerPixel": 1,
    ...
    render_graph.add_edge("TestGauss.color", "PrimalAccumulatePass.input")
    render_graph.add_edge("TestGauss.dColor", "GradAccumulatePass.input")
    ...
    passes = {
        ...
        "gauss_diff_pt": gauss_diff_pt_pass,
    }
...
def render_primal(spp: int, testbed: falcor.Testbed, passes):
    passes["gauss_diff_pt"].run_backward = 0
...
def render_grad(spp: int, testbed: falcor.Testbed, passes, dL_dI_buffer, grad_type):
    passes["gauss_diff_pt"].run_backward = 1
    passes["gauss_diff_pt"].dL_dI = dL_dI_buffer

    scene_gradients = passes["gauss_diff_pt"].scene_gradients
